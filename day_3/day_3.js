const engine = () => {
    ...
    const str =".........426.............985.........40..........207............................841..463................................633........17.384... " +
    ...
    const input = str.split(" ").filter((n) => n);
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        for (let j = 0; j < arr[i].length; j++) {
            ...
            if (Number.isInteger(parseInt(arr[i][j]))) {
                if (!startedNumber) {
                    startingIndex = j > 0 ? j - 1 : j;
                }
                startedNumber = true;
                number += arr[i][j];
            } else if (startedNumber && !finishedNumber) {
                finishedNumber = true;
            }

            if (finishedNumber && startedNumber) {
                endingIndex = startingIndex + number.length + 2 < arr[i].length ? startingIndex + number.length + 2  : arr[i].length




                const topString = i > 0 ? arr[i - 1].substring(startingIndex, endingIndex) : "";
                const thisString = arr[i].substring(startingIndex, endingIndex).replace(/\d+/g, '.');
                const bottomString = i < arr.length - 1 ? arr[i + 1].substring(startingIndex, endingIndex) : "";


                if( number == 125 || number == 272){
                    console.log(topString,"top", thisString, "this", bottomString, "STRING");
                   // console.log(startingIndex, "startingIndex", endingIndex, "endingIndex", j)
                }

                // if (topString.startsWith("32")){
                //     console.log(arr[i].length, "LENGTH")
                //     console.log(topString, thisString, bottomString, "STRING");
                //     console.log(startingIndex, "startingIndex", endingIndex, "endingIndex")
                // }
                if (hasSymbol(topString) || hasSymbol(bottomString) || hasSymbol(thisString) ) {

                    if(Number.isInteger(parseInt(number))){
                        numbersFromLine.push(parseInt(number))};
                }
      ...
      console.log(numbersFromLine.length, founds.length)

    console.log(founds.filter(el => numbersFromLine.indexOf(el) === -1), "NUMBERS WHICH  SHOULD BE THERE")
    const res = numbersFromLine.reduce((acc, val) => acc + val, 0)
    console.log(res + 389+630+647+756)
