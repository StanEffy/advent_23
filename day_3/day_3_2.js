const engine = () => {
    ...
    const hasSymbol = (str) => {
        return !!(str?.length && str.split("").find((x) => isNaN(x) && x !== "."));
    };
    ...
    for (let i = 0; i < rows; i++) {
        for (let j = 0; j < cols; j++) {
            if(isNaN(input[i][j]) && input[i][j] !== "."){
                symbols.push ( { row: i, col: j})
            }
        }
    }

    const findAdjacentNumbers = (grid, position) => {
        const { row, col } = position;
        const results = new Set();
        const processed = new Set();

        const isDigit = char => char >= '0' && char <= '9';
        const getFullNumber = (r, c) => {
            if (processed.has(`${r},${c}`)) return null;
            ...
            while (i >= 0 && isDigit(grid[r][i])) {
                processed.add(`${r},${i}`);
                number = grid[r][i] + number;
                i--;
            }
            // Move right
            i = c + 1;
            while (i < grid[r].length && isDigit(grid[r][i])) {
                processed.add(`${r},${i}`);
                number += grid[r][i];
                i++;
            }
        ...
        // Function to check and process a cell
        const checkCell = (r, c) => {
            if (r >= 0 && r < grid.length && c >= 0 && c < grid[r].length && isDigit(grid[r][c])) {
                const number = getFullNumber(r, c);
                if (number !== null) {
                    results.add(number);
                }
            }
        ...
        // Check all eight directions: up, down, left, right, and four diagonals
        checkCell(row - 1, col);     // Up
        checkCell(row + 1, col);     // Down
        checkCell(row, col - 1);     // Left
        checkCell(row, col + 1);     // Right
        checkCell(row - 1, col - 1); // Top-left diagonal
        checkCell(row - 1, col + 1); // Top-right diagonal
        checkCell(row + 1, col - 1); // Bottom-left diagonal
        checkCell(row + 1, col + 1); // Bottom-right diagonal

        return Array.from(results);
    };

    const results = symbols.map(s => findAdjacentNumbers(arr, s)).filter( arr => arr.length === 2)
    const sum = results.map(arr => arr.reduce((acc, val) => acc * val, 1)).reduce((acc, val) => acc + val, 0)
    console.log(sum)
