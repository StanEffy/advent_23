    ...
    parsedGames.map((rounds) => {
        ...
        rounds.forEach(r => {
            for (const color in r) {
                if (r[color] > object[color]) {
                    object[color] = r[color]
                }
            }
        })
        ...
        for (const color in object){
            multiplier *= object[color]
        }
        values.push(multiplier)
    })

// Calculate the sum of IDs of valid games
    const sumOfPowers = values.reduce((acc, val, index) => acc + val, 0);

    console.log("Sum of IDs of Valid Games:", sumOfPowers);
