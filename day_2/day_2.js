const cubes = () => {
    const str ="Game 1: 5 red, 1 green; 6 red, 3 blue; 9 red; 1 blue, 1 green, 4 red; 1 green, 2 blue; 2 blue, 1 red " +
    ...
    const set = []
    const gamesData = str.split(/Game \d+: /).filter(Boolean);
    ...
    gamesData.forEach(game => {
        const rounds = game.split(';').filter(Boolean);
        const gameRounds = [];

        rounds.forEach(round => {
            const roundObject = {};
            const colorMatches = round.match(/(\d+) (\w+)/g);

            if (colorMatches) {
                colorMatches.forEach(match => {
                    const [count, color] = match.split(" ");
                    roundObject[color] = parseInt(count);
                });
            }

            gameRounds.push(roundObject);
        });

        parsedGames.push(gameRounds);
    });

    const possibleGames = [];
    const validGames = parsedGames.filter((gameRounds, index) => {
        const isValid = gameRounds.every((round) => {
            for (const color in round) {
                if (round[color] > maxCounts[color]) {
                    return false;
                }
            }
            return true;
        ...
        isValid ? possibleGames.push(index + 1) : null
    });

// Calculate the sum of IDs of valid games
    const sumOfValidGameIDs = possibleGames.reduce((acc, val, index) => acc +  val, 0);

    console.log("Valid Games:", validGames.length);
    console.log("Sum of IDs of Valid Games:", sumOfValidGameIDs);
