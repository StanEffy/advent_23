const learnSum = () => {
  const str2 = "two1nine " +
      ...
      "zoneight234 " +
      "7pqrstsixteen"
  const str = "threerznlrhtkjp23mtflmbrzq395three " +
      "9sevenvlttm " +
      "3twochzbv " +
  ...
  arr.forEach((str, ind) => {
    ...
    arrNum.forEach(key => {
      ...
      if(first !== -1){
       if(leftSide.index > first){
         leftSide.index = first
         leftSide.value = dict[key]
       }
      }

      if(last !== -1){
        if(rightSide.index < last){
          rightSide.index = last
          rightSide.value = dict[key]
        }
      }
    })

    for(let i = 0; i < str.length ; i++){
      if(Number.isInteger(+str[i]) && leftSide.index > i){
        leftSide.value = str[i]
        leftSide.index = i
      }

      if(Number.isInteger(+str[i]) && rightSide.index < i){
        rightSide.value = str[i]
        rightSide.index = i
      }
    }
    ...
    if(leftSide.index !== null ){
      inter += leftSide.value
    }
    if(leftSide.index !== null){
      inter += rightSide.value
    }
    if(inter.length === 1){
      inter += inter
    }
...
totalSum += +(inter)
  ...
  //     console.log(`string number ${ind} is ${str} and two-digits are ${leftSide + rightSide}`)
  //     totalSum += +(leftSide + rightSide)
  //   } else if(leftSide.length === 1){
  //     console.log(`string number ${ind} is ${str} and two-digits are ${leftSide + leftSide}`)
  //     totalSum += +(leftSide + leftSide)
  //   } else {
  //     console.log(`string number ${ind} is ${str} and two-digits are ${rightSide + rightSide}`)
  //     totalSum += +(rightSide + rightSide)
  //   }
  })
